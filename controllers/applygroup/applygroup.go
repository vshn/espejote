...
func (a *Applier) UnmarshalJSONFrom(d *jsontext.Decoder) error {
	...
	case '{':
		...
		a.Resource = u
		if u.GetObjectKind().GroupVersionKind() == applyGroupConfigGVK {
	...
	case '[':
		...
		for i := 0; d.PeekKind() != ']'; i++ {
			...
			if elem.Kind == ApplyGroupConfigKind && i != 0 {
				return fmt.Errorf("ApplyGroupConfig must be the first element in an array")
...
func (a *Applier) Apply(ctx context.Context, cli client.Client) error {
	switch a.Kind {
	case NoopKind:
		return nil
	case ApplyKind:
	...
	case DeleteKind:
		...
		if err := cli.Delete(ctx, a.Resource, a.ResourceDeleteOptions...); err != nil {
